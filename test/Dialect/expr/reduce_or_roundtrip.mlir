...
// for `nsl.reduce_or`. Lowers to `comb.icmp ne %a, 0` at M6.

// CHECK-LABEL: nsl.module @ReduceOrHost
nsl.module @ReduceOrHost {
  // CHECK: %{{.*}} = nsl.constant 1 : !nsl.bits<8>
  %a = nsl.constant 1 : !nsl.bits<8>
  // CHECK: %{{.*}} = nsl.reduce_or %{{.*}} : !nsl.bits<8> -> !nsl.bits<1>
  %r = nsl.reduce_or %a : !nsl.bits<8> -> !nsl.bits<1>
