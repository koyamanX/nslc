...
| **M7** | `nsl-driver` (9); `nslc -emit=verilog` operational end-to-end. **`P-VEN` (vendoring) and `P-VCD` (golden VCDs) MUST land on or before M7 — see below.** | The 4 audited NSL projects (`cpu16`, `mips32_single_cycle`, `ahb_lite_nsl`, `turboV`) compile and simulate equivalently to their `golden/*.vcd` under Icarus and Verilator. Corpus narrowed from originally-projected 7 → 4 per the M7 license audit (see `specs/011-m7-driver-e2e/spec.md` FR-009 amendment 2026-05-12; requires a Principle VI "closed list" constitution amendment — pending). | VI (end-to-end NON-NEGOTIABLE; "Delivery" vendoring; "Reference VCDs"); IX (no bypass); V (determinism) |
| **M8** | `riscv-formal` integration for a re-vendored RISC-V audited project. Formal SUPPLEMENTS the golden VCD; it does not replace it (Principle VI). | The re-vendored RISC-V audited project passes the riscv-formal ISA-compliance suite; result published in CI. (Originally targeted at `rv32x_dev`; that project was dropped from M7 per the license audit and may be re-vendored post-relicense per `CONTRIBUTING.md §2.1`.) | VI (formal clause); IX (CI integration) |
...
| **T3** | `nsl-lsp` skeleton: `initialize`, `textDocument/didOpen`, `didChange`, `publishDiagnostics`, `foldingRange`. **Delivered.** `bin/nsl-lsp` produces live Sema diagnostics in any LSP client; the test gate (`LifecycleSuite::README_TestGate_OpenErrorEditFix`) is green. | LSP integration test: open a file with a Sema error, observe diagnostic; edit, observe re-diagnose. | M3 |
...
> **Status by milestone.** As of **M3**, `-emit=tokens` and
> `-emit=ast` are both implemented end-to-end, and `-emit=ast`
> now runs Sema (`nsl-sema`: SymbolTable + TypeSystem + 29 per-`Sn`
> constraint checks) between parse and printing. `-emit=mlir`
> lands at **M5**, `-emit=hw` at **M6**, and the default
> `-emit=verilog` at **M7**. The full `pp.ebnf` directive set
> + 22-helper compile-time evaluator (M1), the recursive-descent
> parser with full multi-error recovery + parser-note
> disambiguation (M2), and the hybrid Sema pass strategy with
> no-cascade unresolved-name handling (M3) are operational.

```bash
# M3 quick check: post-Sema -emit=ast surfaces resolved-type and
# decl-loc enrichments per specs/006-m3-sema/contracts/
# emit-ast-format.contract.md (Q2 Option A — re-cut format).
...
# -> (CompilationUnit ... (ModuleBlock ... name=hello
#      (RegDecl ... name=q : BitVector(8) ...)         <-- type suffix (M3)
#      (FuncDefn ... name=clk
#        (TransferStmt ... op=RegColonEq
#          (IdentifierExpr ... : BitVector(8) -> decl@/tmp/hello.nsl:2:7
#                              name=q)                   <-- decl-loc backref (M3)
#          ...)))
# See specs/006-m3-sema/quickstart.md for the full M3 walkthrough.
```
