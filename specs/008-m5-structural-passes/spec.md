...
- **FR-006**: Every concrete `visit(...)` override on the visitor MUST produce the `nsl::*` op named in design §8's lowering table (lines 1057–1078). Each row of that table maps to a frozen "MUST" requirement here:
  - `ModuleBlock` → `nsl.module` (port list built from associated `DeclareBlock`; control-terminal port-info ops emitted into module body, data-port-info ops dual-placed into both module body and the paired `nsl.declare` body per post-merge M4-amendment 2026-05-05 #9)
  - `DeclareBlock` (post-merge M4-amendment 2026-05-05 #9) → `nsl.declare @name { nsl.input_port / nsl.output_port / nsl.inout_port ... }` (port-list metadata for M6 `hw::HWModuleOp` derivation) + dual-placed port-info ops inside the matching `nsl.module`'s body (SSA-Value-bearing for transfer resolution; SSA-dominance forces the dual emission). Control terminals (`func_in`/`func_out`/`func_self`) lower to `nsl.func_in`/`nsl.func_out`/`nsl.func_self` in `nsl.module`'s body (those ops carry `HasParent<"ModuleOp">`). `Wire`-direction dummy args inside a declare are skipped (no `nsl.wire_port` op; Sema's S4 enforces direction)
