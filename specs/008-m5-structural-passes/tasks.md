...
- [ ] T042 [P] [US1] Author `test/Lower/expr/{unary,literal,identifier,conditional,slice,concat}_emit_mlir.nsl` covering the remaining expression-position `ast::*Expr` kinds (PARTIAL: `unary` shipped Phase B Commit 2 — also covers `SignExtendExpr` + `ZeroExtendExpr`; `literal` + `identifier` covered indirectly via `transferstmt_*` + `regdecl_init`)
