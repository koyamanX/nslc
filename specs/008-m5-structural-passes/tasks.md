...
- [X] T035 [P] [US1] Author `test/Lower/action/forblock_enum_emit_mlir.nsl` + `forblock_cstyle_emit_mlir.nsl` covering both `ast::ForBlock` shapes (C-style only — enum form escalated; M4 `nsl.for` is 3-operand-only, spec.md:419 maps enum form to a 2-operand op that doesn't exist on the frozen-77 surface) — *ENUM FORM CLOSED offload-2026-04-30 (M4 amendment #5 + M5 visitor consumption): amendment #5 widened `nsl.for`'s `step` to `Variadic<NSL_BitsOrStruct>` (commit `b46a7d1`); `visit(ForBlock)`'s enum-form arm now lowers `ForForm{init, cond, step=nullptr}` to the 2-operand op shape via `ValueRange{}` for `step`. End-to-end fixture `forblock_enum_emit_mlir.nsl` is GREEN.*
...
- [X] T045 [P] [US1] Author `test/Lower/marker/fire_probe_emit_mlir.nsl` covering control-name-as-1-bit-value (S27) → `nsl.fire_probe` (M4-valid subset only — `func_in`/`func_out`/`func_self`; ProcName/StateName escalated per fire_probe verifier `NSLOps.cpp:792-822`) — *PROC_NAME FLAVOUR CLOSED offload-2026-04-30 (M4 amendment #5 + M5 visitor consumption): amendment #5 widened `FireProbeOp::verify` to accept sibling `nsl.proc` (module scope) and sibling `nsl.state` (per-proc scope) via `mlir::SymbolTable::lookupSymbolIn` (commit `b46a7d1`); `visit(ProcNameDecl)` and `visit(StateNameDecl)` (formerly STUBs) now register names in `controlTable_` with an AST-side conditional check (only when a matching ProcDefn / StateDefn body exists in the same module / proc — preserves `test/sema/s27/pass.nsl` baseline which has proc_name without a body). End-to-end fixture `fire_probe_proc_name_emit_mlir.nsl` is GREEN.*
...
- [X] T077 [P] [US3] Author `test/Lower/passes/nsl-expand-variables/cross_scope.mlir` per US3 fixture-axis (e) (variable in func consumed in proc) — *delivered offload-2026-04-30 Commit 1 (180a225) as **XFAIL** + deferred; **CLOSED offload-2026-04-30 (M4 amendment #5 + M5 consumption)**: `nsl.wire`'s parent trait was widened to `ParentOneOf<["ModuleOp", "FuncOp"]>` in amendment #5 (commit `b46a7d1`), and `NSLExpandVariablesPass`'s scope guard was widened to admit `FuncOp` parents (this commit). Fixture re-authored with a `nsl.constant` + `nsl.transfer` body so the wire is materialised by the per-version walk; XFAIL marker removed, GREEN.*
...
- [X] T085 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/residue_typo.mlir` per US4 acceptance scenario 1 + `residue-detection.contract.md` §8 row 1 — *delivered offload-2026-04-30 Commit 2; uses `--verify-diagnostics` + `expected-error@+1` annotation pattern (precedent: `test/Dialect/action-block/if_invalid_*.mlir`).*
- [X] T086 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/residue_undefined.mlir` per `residue-detection.contract.md` §8 row 2 — *delivered offload-2026-04-30 Commit 2; covers entire-string-is-token edge case (no surrounding text).*
- [X] T087 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/residue_multi.mlir` per US4 acceptance scenario 3 (two diagnostics on different lines) — *delivered offload-2026-04-30 Commit 2; covers BOTH multi-token-in-one-StringAttr (`"%X%_%Y%"` → 2 diags) AND multi-op cases. `std::regex_iterator` finds non-overlapping matches deterministically.*
- [X] T088 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/s15_post_param.mlir` per `pass-pipeline.contract.md` §3 row S15 — *delivered offload-2026-04-30 Commit 3 as XFAIL; S15 is VACUOUS at M5's frozen 79-op surface (`nsl.slice` carries `I64Attr` lo/hi, not operand-side SSA values). Helper body lands when an op variant whose index is operand-side `Value` is added.*
- [X] T089 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/s16_pure_nsl.mlir` per `pass-pipeline.contract.md` §3 row S16 — *delivered offload-2026-04-30 Commit 3; structural check fires on every surviving `nsl.param_int` / `nsl.param_str` (heuristic: at M5 ALL submodules are pure-NSL so any surviving param qualifies).*
- [X] T090 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/s25_replicated_collision.mlir` per `pass-pipeline.contract.md` §3 row S25 — *delivered offload-2026-04-30 Commit 3; structural check walks each `nsl.module` body for duplicate `name` `StringAttr` on `nsl.{reg,wire,variable,mem}` ops via `llvm::StringMap` source-order tracking. Diagnostic fires on the SECOND occurrence.*
- [X] T091 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/{s6_use_before_def,s10_loop_var_residue,s20_submod_array_iface}.mlir` for the remaining sensitive-`Sn` rows — *delivered offload-2026-04-30 Commit 3; S10 is the only one that ships GREEN (structural check via `module.walk<StructuralGenerateOp>` — fires on any `structural_generate` surviving to slot 6). S6 (XFAIL) requires SSA operand-traversal across regions — exceeds slot-6 single-pass budget. S20 (XFAIL) is post-M6 surface (interface-modifier bindings are M6 dialect).*
- [X] T092 [P] [US4] Author `test/Lower/passes/nsl-check-semantics/clean_baseline.mlir` per `residue-detection.contract.md` §8 last row (positive case — round-trip success, zero diagnostics) — *delivered offload-2026-04-30 Commit 4; well-formed module with two-module setup (`@CleanBaseline` + `@AnotherCleanModule`), zero residue, zero generate, zero param, distinct decl names. Exit 0 + IR preserved verbatim. Locks in the FALSE-POSITIVE contract (residue-detection.contract.md §5).*
- [X] T093 [P] [US4] Author `test/Lower/residue/typo_undefined_emit_mlir.nsl` end-to-end fixture (NSL source with `%TYPO%` → driver exits non-zero with the expected diagnostic on stderr) per US4 acceptance scenario 1 — *delivered offload-2026-04-30 Commit 5 as XFAIL; M1 preprocessor rejects `%TYPO%` at lex time (`error: undefined macro reference: '%TYPO%'`) before the parser produces the AST shape that the visitor would lower to `nsl.reg "buf_%TYPO%"`. Same M1 blocker as US2 T066/T067 — the residue never reaches the slot-6 check via pure-NSL source. Pass-standalone fixtures (T085-T087) cover the slot-6 mechanics fully — that's the substantive deliverable. End-to-end coverage waits on M1 preprocessor amendment per offload task #23.*
- [X] T094 [US4] Run lit suite; confirm T084–T093 all FAIL (RED phase — explode-submod-array + check-semantics still no-ops) — *delivered offload-2026-04-30 across Commits 1-3 (per-commit RED-then-GREEN cycle for each fixture batch). RED baselines confirmed via lit invocation against the no-op pass before each implementation commit.*
...
- [X] T096 [US4] Implement `NSLCheckSemanticsPass::runOnOperation` body in `lib/Lower/Pass/NSLCheckSemanticsPass.cpp` — regex scan over `mlir::StringAttr` values per `residue-detection.contract.md` §1 + §2 (turns T085–T087, T093 GREEN) — *delivered offload-2026-04-30 Commit 2; `std::regex` (function-local-static + C++11 magic statics) over both `mlir::StringAttr` AND `mlir::FlatSymbolRefAttr` named-attribute slots per `residue-detection.contract.md` §3. Multi-error within one pass-invocation (signalPassFailure IFF `diagCount > 0`); diagnostic count summed across all walks then committed at end. Frozen diagnostic string per FR-018: `error: unresolved macro splice '%<IDENT>%' after structural expansion`.*
- [X] T097 [US4] Extend `NSLCheckSemanticsPass` body with the six sensitive-`Sn` re-check helpers per `pass-pipeline.contract.md` §3 (S6, S10, S15, S16, S20, S25 — diagnostic strings frozen) (turns T088–T091 GREEN) — *delivered offload-2026-04-30 Commit 3; THREE substantive structural-only helpers landed (S10/S16/S25) + THREE documented stubs (S6/S15/S20). Per offload guidance: "If any Sn re-check requires non-trivial dataflow analysis... simplify to a structural check that catches the common case + document the limitation." Each stub documents the deferral reason inline + has an XFAIL'd fixture preserving the diagnostic-string contract for future amendment. Frozen diagnostic strings used verbatim per Principle VIII.*
- [X] T098 [US4] Verify `clean_baseline.mlir` passes with zero diagnostics (T092 GREEN) — *delivered offload-2026-04-30 Commit 4; verified `nsl-opt -nsl-check-semantics clean_baseline.mlir` exits 0 with IR preserved. SC-006 positive-case gate ships.*
- [X] T099 [US4] Run lit suite; confirm T084–T093 all GREEN; verify `nslc -emit=mlir typo.nsl` exits non-zero AND emits zero `.mlir` output AND emits the FR-018 diagnostic on stderr (per SC-006 + US4 acceptance scenario 1) — *delivered offload-2026-04-30 Commit 6; lit 497/504 PASS + 7 XFAIL. Pass-standalone path GREEN for all 4 substantive scenarios (residue typo/undefined/multi + clean_baseline) + 3 of 6 sensitive-Sn (S10/S16/S25). End-to-end via `nslc -emit=mlir typo.nsl` (T093) DEFERRED on M1 preprocessor blocker (XFAIL'd; same precedent as US2 T066/T067). Determinism check: `diff -q` of two consecutive `nsl-opt -nsl-explode-submod-array` and `nsl-opt -nsl-check-semantics` runs reports BYTE-STABLE.*

**Checkpoint**: After T099, US4 ships **pass-standalone (full)** + sensitive-Sn (partial). Residue cases produce sourced diagnostics through the slot-6 helper (3/3 substantive fixtures GREEN); 3 of 6 sensitive-`Sn` constraints land real structural-only re-check helpers (S10/S16/S25 GREEN); the other 3 (S6/S15/S20) are documented stubs + XFAIL'd fixtures preserving the frozen diagnostic-string contract for future M5+ amendments. The full pipeline (all six pass slots with real bodies for slots 1, 2, 3, 4, 6 + a documented no-op slot 5) is operational. US1/US2/US3 fixtures remain GREEN; smoke 33+ round-trip-clean; determinism BYTE-STABLE.

**ACTUAL STATUS at offload-2026-04-30 close (Commits d49650e..41b74bc + tasks-update commit)**: pass-standalone path FULLY FUNCTIONAL for residue (T085-T087 + T092 + T096 GREEN), explode-submod-array (T084 + T095 GREEN), and 3/6 sensitive-Sn (T088-T091 + T097 — S10/S16/S25 GREEN; S6/S15/S20 XFAIL'd). End-to-end residue path (T093) DEFERRED on M1 preprocessor blocker. US4 ships the structural deliverable (both pass bodies real + per-pass fixtures + clean_baseline gate); the end-to-end NSL-source path lands when the M1 preprocessor amendment from US2 task #23 lands. The deferred-Sn helpers (S6/S15/S20) land when their M4+ amendment surfaces materialise (operand-traversal infra / operand-side index op variant / M6 iface-modifier dialect).
...
- [X] T100 [P] [US5] Author `test/Lower/determinism/canonical_smoke.nsl` — a single non-trivial fixture exercising US1+US2+US3 features (a `module` with a `proc`, a `generate`, a `variable`, several `Sn`-relevant patterns) — landed Commit 1 of US5 offload (`generate` excluded per M1 `%IDENT%` blocker, documented in fixture comments; `goto` STUB-visitor, func-scope variable per US3 deferral also excluded; module-scope variable + reg/wire/mem + func+seq + proc+state+finish + binary-expr in clocked-transfer cover the cross-section)
- [X] T101 [US5] Author `scripts/determinism_check.sh` — builds the project twice in `$TMPDIR/build-det-a` + `$TMPDIR/build-det-b` (distinct host paths), runs `nslc -emit=mlir` on T100 in each, `diff -q`s the outputs, exits non-zero on mismatch; also greps each output for the forbidden patterns from `driver-emit-mlir.contract.md` §3 (`/build`, `/home`, `/tmp`, `0x[0-9a-fA-F]{8,}`, time-of-day patterns) — landed Commit 2 of US5 offload; uses tar (not rsync — absent from dev image) for source-tree copy; ASan disabled in both build cells per M4 `3326eb6` precedent

### Implementation for User Story 5

- [X] T102 [US5] Author `scripts/audit_determinism.sh` per [`research.md`](./research.md) §13 — greps `lib/Lower/` for forbidden patterns (`std::unordered_`, `reinterpret_cast<uintptr_t>`, `std::time`, `std::chrono::`, etc.); CI-blocking on match — landed Commit 1 of US5 offload; 12 patterns scanned (all `std::unordered_*` flavours, `reinterpret_cast<uintptr_t>`, `std::time(`, `std::chrono::`, `std::random_*`, `std::mt19937`, `getpid()`, `gettimeofday(`); zero forbidden matches at HEAD
- [X] T103 [US5] Wire T101 + T102 into `scripts/ci.sh` stage 2 (static checks); add a CI matrix entry that runs T101 on every PR (per FR-029 + SC-007) — landed Commit 2 of US5 offload; stage 2 gains step 6 (audit_determinism — runs unconditionally) + step 7 (determinism_check — opt-in via `NSLC_RUN_DETERMINISM_CHECK=1`, expensive `~minutes` cross-path build)
- [X] T104 [US5] Run `bash scripts/determinism_check.sh` locally; if non-empty diff, debug per [`quickstart.md`](./quickstart.md) §6 (most common cause: debug-info attrs, build-path-derived diagnostic locations); iterate until empty diff (per SC-007 + SC-008) — landed Commit 3 of US5 offload; PASS first try (byte-identical output across `/tmp/nslc-det-a-7` vs `/tmp/nslc-det-bbb-7`; zero forbidden host-path/timestamp/pointer-address patterns); audit_determinism PASS (zero forbidden patterns in lib/Lower/)
...
- [X] T106 [P] Bulk-author `test/Lower/m3_corpus/s<NN>/<case>.expected.mlir` for all ~34 Sema-clean M3 pass-case fixtures via `nslc -emit=mlir test/sema/<sn>/<case>.nsl > test/Lower/m3_corpus/s<NN>/<case>.expected.mlir` then commit; CI runs `diff` on each in lit (per FR-030 + SC-003) — bulk fixture authoring; can be automated with a one-shot script — *post-merge offload close-out 2026-04-30: 4 of the 5 XFAIL'd corpus fixtures (s07/s08/s19/s25) flipped to GREEN by M4 amendment #6 (single trait swap on `nsl.seq` aligning the parent constraint with NSL S7). Goldens regenerated via `nslc -emit=mlir test/sema/s<NN>/pass.nsl > test/Lower/m3_corpus/s<NN>/pass.expected.mlir`; standard `diff` form restored on the four pass.test files. The fifth (s09) hits a separate, pre-existing M5-side blocker — `for` loop's literal `8` overflows the lowerExpr 1-bit-default width — XFAIL preserved with updated banner pointing at T055 follow-up. lit XFAIL count drops from 13 to 9 (-4).* — *post-merge fix-bug offload 2026-04-30 close-out: s09 closed by adding `lowerExpr` `mlir::Type typeHint` parameter (LHS-first lowering propagating result type to RHS for SameOperandsAndResultType + SameTypeOperands trait shapes), plus sized-literal spelling parser (`2'd0`, `4'b0000`, `8'h2A`) that reads the explicit width verbatim. Same fix unblocks 7 of 9 failing `examples/*.nsl` (only `01_hello`/`20_simulation_tb` remain, both on `nsl.sim_finish` parent constraint inside `nsl.sim_init` — Category B amendment territory, deferred). Golden regenerated; XFAIL marker removed. lit count: 535 PASS + 9 XFAIL → 536 PASS + 8 XFAIL.* — *post-merge offload close-out 2026-04-30 (amendment #7): the 2 remaining examples (`01_hello.nsl` + `20_simulation_tb.nsl`) closed by single trait swap on `nsl.sim_finish` (`HasParent<"ModuleOp"> → ParentOneOf<["ModuleOp", "SimInitOp"]>`) aligning the dialect with NSL S29 (`lang.ebnf §10` line 1007 — `_init { _finish(); }` is the canonical termination idiom). Sibling `nsl.sim_display` / `nsl.sim_delay` already accepted `SimInitOp` parent. **Entire 20-example NSL corpus now compiles cleanly via `nslc -emit=mlir` — zero FAILs.** lit count: 536 PASS + 8 XFAIL → 537 PASS + 8 XFAIL (+1 from the new round-trip fixture `test/Dialect/system-task/sim_finish_in_sim_init_roundtrip.mlir`).*
...
- [X] T109 Run `nslc -emit=tokens` and `nslc -emit=ast` on the full M3 corpus; `diff -q` outputs against an M4-baseline run; confirm zero changes (regression-guard per SC-010)
- [X] T110 [P] Author `scripts/audit_op_locations.sh` per FR-008 + SC-009 — runs `nsl-opt -mlir-print-debuginfo` on every `.mlir.expected` golden under `test/Lower/**/` and on every `nslc -emit=mlir` CI output, greps for the literal token `loc(unknown)`, exits non-zero on any match. The same script asserts at least one `FusedLoc(...)` shape on `test/Lower/expr/structcastexpr_emit_mlir.mlir.expected` (one composite-expression fixture per category) covering the FR-008 multi-source `mlir::FusedLoc` clause. Wire into `scripts/ci.sh` stage 2 (static checks). Closes coverage gaps surfaced by `/speckit-analyze` 2026-04-30 findings A3 + A4. **Shipped at M5 in soft-fail mode** (option `(a)` per offload guidance): the visitor uses `builder_.getUnknownLoc()` universally pending the SourceManager <-> mlir::MLIRContext location-translator adapter (post-M5 amendment); script is wired into ci.sh stage 2 step 8 with a deferred-status banner that exits 0 unless `NSLC_RUN_LOCATION_AUDIT=1` opts into real enforcement.
- [X] T111 Run [`quickstart.md`](./quickstart.md) §10 pre-merge checklist end-to-end — every box ticked; CI green on all six Principle IX stages; CodeRabbit blocking findings addressed
