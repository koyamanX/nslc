...
- [ ] T038 [US2] Implement `OpConversionPattern<nsl::ModuleOp>` in `lib/Lower/CIRCTPatterns/ModulePatterns.cpp` — walks the paired `nsl::DeclareOp` (matched by `pair_name == sym_name` literal-string equality across the parent `mlir::ModuleOp`'s direct children, post-merge M4-amendment #9) to build the port list from the declare body's `nsl::InputPortOp` / `nsl::OutputPortOp` / `nsl::InoutPortOp` children, creates `hw::HWModuleOp`, recursively converts the body region. The in-module port-info ops (dual-placement per amendment-#9) are rewritten as block-arg substitutions (`InputPortOp`) / output-port wiring (`OutputPortOp`) / bidirectional wiring (`InoutPortOp`) — see PortPatterns (T038b). Per [`contracts/circt-lowering.contract.md`](./contracts/circt-lowering.contract.md) §3 port-list derivation rules. Implicit `clk` + `rst_n` appended for no-`interface` modules. (T030–T035 fixtures pass after this lands)
- [ ] T038b [US2] Implement `OpConversionPattern<nsl::InputPortOp>` / `OpConversionPattern<nsl::OutputPortOp>` / `OpConversionPattern<nsl::InoutPortOp>` in `lib/Lower/CIRCTPatterns/PortPatterns.cpp` — for the in-module-body placement: rewrite each op's result with the corresponding `hw::HWModuleOp` block-arg / output-wire. For the declare-body placement: the parent ModuleOp pattern already consumed them during port-list derivation; the in-declare pattern is a no-op rewriter that erases the op (or marks it legal-and-erased). Per amendment-#9 dual-placement contract.
