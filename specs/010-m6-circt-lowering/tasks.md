...
- [ ] T001 Verify M5 baseline build is green on master HEAD via `sg docker -c "docker run --rm -v $PWD:/workspace -w /workspace ghcr.io/koyamanx/nsl-nslc:dev bash -c 'cmake -G Ninja -B build-noasan -DCMAKE_BUILD_TYPE=Debug -DNSL_ENABLE_ASAN=OFF && ninja -C build-noasan check-nsl'"` — record the baseline pass count for regression-comparison post-M6
- [ ] T002 [P] Create `test/Lower/circt/` subdirectory tree: `module/`, `fsm/`, `arith/`, `state/`, `control/`, `sim/`, `round_trip/` per [`data-model.md`](./data-model.md) §8
- [ ] T003 [P] Amend `test/Lower/lit.cfg.py` to register `test/Lower/circt/` test directory hierarchy + ensure `nslc`, `nsl-opt`, and `circt-opt` (vendored from CIRCT) are on the lit `PATH` for these tests
- [ ] T004 [P] Author `test/Lower/circt/coverage_guard.cmake` skeleton — walks `lib/Lower/CIRCTPatterns/*.cpp` for registered pattern types; walks `test/Lower/circt/<family>/*.nsl` for fixture names; fails configure if bijection breaks (FR-033). Fail empty initially (no patterns, no fixtures); will fire green as US2/US3/US4 fill in.
...
- [ ] T005 Amend `include/nsl/Lower/Lower.h` to add the 2 new public symbols (`createNSLToCIRCTPass`, `registerNSLToCIRCTPass`) frozen by [`contracts/lower-api.contract.md`](./contracts/lower-api.contract.md) §2 — declarations only, no bodies
- [ ] T006 [P] Author `lib/Lower/NSLToCIRCTPass.h` (private internal header) with the `NSLToCIRCTPass` class declaration per [`data-model.md`](./data-model.md) §1 — `mlir::PassWrapper<NSLToCIRCTPass, mlir::OperationPass<mlir::ModuleOp>>` skeleton with `getArgument()`, `getDescription()`, `getDependentDialects()`, `runOnOperation()` declarations
- [ ] T007 [P] Author `lib/Lower/CIRCTTypeConverter.h` + `CIRCTTypeConverter.cpp` per [`data-model.md`](./data-model.md) §2 — the two `addConversion` calls (`!nsl.bits<W>` → `iW`, `!nsl.struct<@T>` → packed `iN`), source/target materialisation lambdas, `computeStructPackedWidth` helper
- [ ] T008 [P] Author `lib/Lower/CIRCTPatterns/ModulePatterns.cpp` with empty `populateModulePatterns(RewritePatternSet&, CIRCTTypeConverter&)` function — patterns land in T024+
- [ ] T009 [P] Author `lib/Lower/CIRCTPatterns/PortPatterns.cpp` with empty `populatePortPatterns` — patterns land in T028+
- [ ] T010 [P] Author `lib/Lower/CIRCTPatterns/StatePatterns.cpp` with empty `populateStatePatterns` — patterns land in T070+
- [ ] T011 [P] Author `lib/Lower/CIRCTPatterns/ControlPatterns.cpp` with empty `populateControlPatterns` — patterns land in T080+
- [ ] T012 [P] Author `lib/Lower/CIRCTPatterns/FSMPatterns.cpp` with empty `populateFSMPatterns` — patterns land in T040+
- [ ] T013 [P] Author `lib/Lower/CIRCTPatterns/ArithPatterns.cpp` with empty `populateArithPatterns` — patterns land in T060+
- [ ] T014 [P] Author `lib/Lower/CIRCTPatterns/BitOpPatterns.cpp` with empty `populateBitOpPatterns` — patterns land in T065+
- [ ] T015 [P] Author `lib/Lower/CIRCTPatterns/SimPatterns.cpp` with empty `populateSimPatterns` — patterns land in T085+
- [ ] T016 [P] Author `lib/Lower/CIRCTPatterns/ParamPatterns.cpp` with empty `populateParamPatterns` — patterns land in T030+
- [ ] T017 Author `lib/Lower/NSLToCIRCTPass.cpp` body — `runOnOperation` constructs `ConversionTarget` (illegal `nsl` dialect, legal 5 CIRCT dialects), constructs `CIRCTTypeConverter`, populates `RewritePatternSet` by calling all 9 `populate*Patterns` helpers in alphabetic order per [`research.md`](./research.md) §11, calls `applyFullConversion`. On failure, calls `signalPassFailure()` after the `DiagnosticBridge` translates MLIR diagnostics. Uses M5's `DiagnosticBridge` from `lib/Lower/DiagnosticBridge.h` unchanged (depends on T006, T007, T008–T016)
...
- [ ] T025 [P] Amend `tools/nslc/main.cpp` `--help` text to include `-emit=hw` (and `-emit=circt` alias) lines per [`contracts/driver-emit-hw.contract.md`](./contracts/driver-emit-hw.contract.md) §1

**Checkpoint**: After T025, the dev-container build produces a working `nslc` and `nsl-opt`; `nslc -emit=hw input.nsl` runs the full pipeline (M1 → M2 → M3 → M5 → M6 with empty pattern set) and observed-fails on any non-trivial input with `error: nsl→CIRCT conversion failed for op '<...>'`. `nsl-opt --help` lists `-nsl-to-circt`. The foundation is ready; user-story implementation can now begin.
