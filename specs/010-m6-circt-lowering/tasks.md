...
- [X] T033 [P] [US2] Author `test/Lower/circt/module/interface_modifier.nsl`. **DEFERRED to follow-on M4 amendment-#10 (XFAIL'd 2026-05-04 in this commit).** At amendment-#9 the S20 `interface` modifier is NOT surfaced on `nsl::DeclareOp` (the dialect carries no `interface_clock` / `interface_reset` attribute pair); reading the modifier from M3 Sema's introspection observable would require plumbing Sema results through `Compilation::lowerToCIRCT` (significant scope creep). Fixture authored with `// XFAIL: *` so it surfaces in `coverage_guard.cmake`'s scan but doesn't block CI; flips to PASS in the amendment-#10 PR.
