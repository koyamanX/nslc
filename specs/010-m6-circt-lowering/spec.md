...
- **FR-026**: `nslc -emit=hw -` (read from stdin) **MUST be supported**, matching the convention used by `-emit=tokens` / `-emit=ast` / `-emit=mlir` from M3/M5. **Status: implemented post-M6** via a small follow-on PR that landed `mkstemp`-trampoline stdin support in `tools/nslc/main.cpp` for every `-emit=<stage>`. The fixture `test/Lower/circt/round_trip/stdin_pipe.test` (T139) flipped XFAIL → PASS in the same PR. Closes the C2 finding from /speckit-analyze.
