...
| **II. Layered Library Architecture** | Yes | ✅ | All implementation work lives in `lib/Preprocess/` (layer 2). No new layers. The new `MacroExpander.{h,cpp}` is private to `lib/Preprocess/` (no public header in `include/nsl/Preprocess/`); follows the M1 convention where `DirectiveParser`/`PPExpression`/`IdentSplicer` are also private. `tools/nslc/main.cpp` unchanged (no new CLI flags); 60-line cap preserved. |
