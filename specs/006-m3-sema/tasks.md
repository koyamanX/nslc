...
- [ ] T002 Sanity-verify the M0+M1+M2 build is still green on branch `006-m3-sema` by running `cmake --build build && ctest --test-dir build --output-on-failure && lit -v test` inside `ghcr.io/koyamanx/nsl-nslc:dev` against the unchanged-since-M2 source tree (checkpoint before adding M3 sources). **Deferred** to user's local CI run — needs docker (sandbox-disabled bash) and ~5+ minute build cycle from scratch (no `build/` directory currently present); not run in the Phase-1 session per agreed scope (same precedent as M2's T002 deferral). Will be exercised implicitly when the first Phase-2 source lands and the agent runs the full local CI.
...
**Checkpoint** (reached 2026-04-28): M3 directory skeleton in place (T001 done; 38 `.keep` files); build sanity verification deferred to first Phase-2 CI run (T002 deferred); layering guard not needed — M0 macro already enforces SC-009 (T003 DEFERRED, rationale above). Phase 2 work can begin.
