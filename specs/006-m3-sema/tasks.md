...
- [ ] T001 Create the M3 test directory tree under `test/` and `test_unit/`: `test/sema/{s01,s02,s03,s04,s05,s06,s07,s08,s09,s10,s11,s12,s13,s14,s15,s16,s17,s18,s19,s20,s21,s22,s23,s24,s25,s26,s27,s28,s29,recovery,resolution,width,emit-ast-resolved}/` and `test_unit/{symbol_table_test,type_system_test,resolution_pass_test,constructive_sn_test,sema_lifecycle_test}/` — each with a `.keep` placeholder so M0's lit-discovery picks them up.
- [ ] T002 Sanity-verify the M0+M1+M2 build is still green on branch `006-m3-sema` by running `cmake --build build && ctest --test-dir build --output-on-failure && lit -v test` inside `ghcr.io/koyamanx/nsl-nslc:dev` against the unchanged-since-M2 source tree (checkpoint before adding M3 sources).
...
**Checkpoint**: M3 directory skeleton in place; build sanity verified; layering guard inherits from M0. Phase 2 work can begin.
