...
- [X] T023 [P] [US1] Done (representative) — `test/sema/resolution/module_scope.test` asserts `→ decl@<file>:<line>:<col>` suffix on RegSymbol references. Full per-scope-kind / per-scoped-name-form coverage scoped down to one representative fixture for Phase 3 MVP; broader fixtures land alongside Phase 4 per-Sn corpus.
