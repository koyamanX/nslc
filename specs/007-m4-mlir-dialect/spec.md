...
### Session 2026-04-30 (post-merge amendment #7)

- Q: M5 example-suite close-out triage identified the last 2 of the originally-9 failing examples (`examples/01_hello.nsl` + `examples/20_simulation_tb.nsl`) hit the SAME dialect-side blocker — `error: 'nsl.sim_finish' op expects parent op 'nsl.module'`. Root cause: `nsl.sim_finish` carried `HasParent<"ModuleOp">` (NSLOps.td:1105 pre-amendment), while NSL spec **S29** (`lang.ebnf §10` line 1007) explicitly permits the `_init { _finish(); }` idiom — every termination-bearing testbench in the example corpus uses this exact shape. Sibling sim-ops (`nsl.sim_display`, `nsl.sim_delay`) already carried `ParentOneOf<["ModuleOp", "SimInitOp"]>` (Phase 3 corpus precedent); only `nsl.sim_finish` was stricter than NSL grammar permitted. Choose between (A) emit CIRCT-side helpers directly from M5 (violates Principle III), (B) amend M4 with the single trait relaxation `HasParent<"ModuleOp"> → ParentOneOf<["ModuleOp", "SimInitOp"]>` (no new ops; mirrors amendment #6's shape), (C) defer the 2 examples to M6 (semantically opaque IR; breaks the M5→M6 cut), (D) downgrade `nsl.sim_finish` to a stub-only op (postpones the cut and breaks Principle VIII test sequencing). → A: **Option (B) — amend M4 with the single trait relaxation.** Rationale: same as amendments #3, #5, and #6 — a "strictly accept more" trait relaxation aligning the dialect parent constraint with NSL grammar's S29 permission; the prior strict trait was stricter than NSL allows; the alternatives (A)/(C)/(D) all violate either Principle III or Principle VIII. Cost: **zero new op records, zero new verifier code, zero hand-verifier additions** — single TableGen `HasParent`/`ParentOneOf` swap. Plus 1 new round-trip fixture (`test/Dialect/system-task/sim_finish_in_sim_init_roundtrip.mlir`) covering the `_init { _display; _delay; _finish; }` shape. Pre-existing `sim_finish_roundtrip.mlir` (immediate-parent `nsl.module`) and `sim_finish_invalid_wrong_parent.mlir` (placing `nsl.sim_finish` directly under the builtin `mlir::ModuleOp`) both continue to PASS / fail-as-expected respectively (the relaxation is purely additive; the builtin top-level `mlir::ModuleOp` is still not in the parent set). Freeze surface stays at **79** (no new op records); SC-012's "next op" baseline unchanged ("73rd op"). The 2 remaining failing examples (`examples/01_hello.nsl`, `examples/20_simulation_tb.nsl`) flip to GREEN in the same offload, closing the entire 20-example suite (0 FAILs). CIRCT-side conversion code does NOT land at M4 — design §10 documents the M6 mapping per-op (Principle III firewall). Cross-reference: `specs/008-m5-structural-passes/research.md` §23 documents the M5-side reasoning.

### Session 2026-04-30 (post-merge amendment #6)

- Q: M5 close-out triage identified that 5 M3-corpus fixtures (`test/Lower/m3_corpus/s{07,08,09,19,25}/pass.test`) were XFAIL'd at HEAD even though `visit(SeqBlock)` is fully implemented (`lib/Lower/ASTToMLIR.cpp:565`). Root cause: `nsl.seq` carried `HasParent<"FuncOp">` (NSLOps.td:884) while NSL spec **S7** (`lang.ebnf §8 line 850`) explicitly permits `seq` inside a *function or procedure* body. The 5 fixtures place `seq` directly inside `nsl.proc` (s07/s08/s09) or inside `nsl.state` (s19/s25 — labelled `goto` form, where the proc body is a state machine that wraps a `seq` in each state). The verifier rejection blocked all 5 from regenerating their goldens. Choose between (A) emit CIRCT-side helpers directly from M5 (violates Principle III), (B) amend M4 with the single trait relaxation `HasParent<"FuncOp"> → ParentOneOf<["FuncOp", "ProcOp", "StateOp"]>` (no new ops; smallest amendment by line count to date), (C) defer the 5 fixtures to M6 (semantically opaque IR; breaks the M5→M6 cut), (D) downgrade `nsl.seq` to a stub-only op (postpones the cut and breaks Principle VIII test sequencing). → A: **Option (B) — amend M4 with the single trait relaxation.** Rationale: same as amendments #3 and #5 — a "strictly accept more" trait relaxation; the prior strict trait was stricter than NSL grammar's S7 permission; the alternatives (A)/(C)/(D) all violate either Principle III or Principle VIII. Cost: **zero new op records, zero new verifier code, zero hand-verifier additions** — single TableGen `HasParent`/`ParentOneOf` swap. Plus 1 new round-trip fixture (`test/Dialect/action-block/seq_in_proc_roundtrip.mlir`) covering both the proc-direct and via-state placements. Pre-existing `seq_roundtrip.mlir` (immediate-parent `nsl.func`) and `seq_invalid_wrong_parent.mlir` (placing `nsl.seq` directly under `nsl.module`) both continue to PASS / fail-as-expected respectively (the relaxation is purely additive; `nsl.module` is still not in the parent set). Freeze surface stays at **79** (no new op records); SC-012's "next op" baseline unchanged ("73rd op"). 5 M3-corpus XFAIL fixtures (s07/s08/s09/s19/s25) flip to GREEN in the same offload. CIRCT-side conversion code does NOT land at M4 — design §10 documents the M6 mapping per-op (Principle III firewall). Cross-reference: `specs/008-m5-structural-passes/research.md` §21 documents the M5-side reasoning.

### Session 2026-05-02 (post-merge amendment #5)

- Q: M5 close-out of the offload-2026-04-30 deferred bundle (T035, T045, T066/T067, T076/T077, T079) surfaced **five** more structural gaps with the M4 dialect (post-amendment-#4 freeze surface 79): (a) `nsl.for` had a mandatory 3-operand `init/cond/step` shape but the NSL enum form `for (i = 0..N) { ... }` (`lang.ebnf §8`) is naturally 2-operand (step implicit); (b) `nsl.fire_probe` accepted only `func_in`/`func_out`/`func_self` targets but S27 (constructive) lists `proc_name` (`nsl.proc`) and `state_name` (`nsl.state`) too; (c) `nsl.reg` rejected `StructuralGenerateOp` as parent so `generate` bodies couldn't declare per-iteration registers (T066/T067); (d) `nsl.variable` rejected struct types so the FR-015 per-field SSA-split chain in `NSLExpandVariablesPass` had no input shape (T076/T079); (e) `nsl.wire` rejected `FuncOp` as parent so `NSLExpandVariablesPass`'s natural func-scope replacement (the wire) was verifier-rejected (T077). Choose between (A) emit CIRCT-side helpers directly from M5 (violates Principle III), (B) amend M4 with the bundled five trait/type-constraint relaxations (no new ops), (C) defer the deferred items to M6 (semantically opaque IR; breaks the M5→M6 cut), (D) downgrade to stub-only ops (postpones the cut and breaks Principle VIII test sequencing). → A: **Option (B) — amend M4 with the bundled five relaxations.** Rationale: (A) violates Principle III's architectural seam; (C) makes the structural-pass interim IR semantically meaningless for `-emit=mlir`; (D) breaks the FileCheck-on-real-IR test discipline. **Cost: zero new op records** (this is the smallest amendment by op-count delta to date) — five existing-op edits: (1) `nsl.for` `step` operand `NSL_BitsOrStruct → Variadic<NSL_BitsOrStruct>` + `ForOp::verify()` rejects ≥ 2 step operands, (2) `FireProbeOp::verify()` adds two SymbolTable lookups (sibling `nsl.proc` at module scope; sibling `nsl.state` inside the enclosing `nsl.proc`), (3) `nsl.reg` parent trait `ParentOneOf<["ModuleOp", "ProcOp"]> → ParentOneOf<["ModuleOp", "ProcOp", "StructuralGenerateOp"]>`, (4) `nsl.variable` result type `NSL_AnyBits → NSL_BitsOrStruct` + `VariableOp::verify()` accepts struct types, (5) `nsl.wire` parent trait `HasParent<"ModuleOp"> → ParentOneOf<["ModuleOp", "FuncOp"]>`. Plus 6 new round-trip fixtures (`test/Dialect/action-block/for_2operand_roundtrip.mlir`, `test/Dialect/marker/fire_probe_proc_target_roundtrip.mlir`, `test/Dialect/marker/fire_probe_state_target_roundtrip.mlir`, `test/Dialect/expansion-only/structural_generate_with_reg_body_roundtrip.mlir`, `test/Dialect/storage/variable_struct_typed_roundtrip.mlir`, `test/Dialect/storage/wire_in_func_roundtrip.mlir`). Freeze surface stays at **79** (no new op records); SC-012's "next op" baseline is unchanged ("73rd op"). The pre-existing fixtures (3-operand `for_roundtrip.mlir`, module-scope `wire_roundtrip.mlir`, bit-only `variable_roundtrip.mlir`, original `fire_probe_roundtrip.mlir`, classic `structural_generate_roundtrip.mlir`) remain valid and continue to PASS — the relaxations are strictly "accept more"; existing invalid fixtures continue to fail-as-expected. CIRCT-side conversion code does NOT land at M4 — design §10 documents the M6 mapping per-op (Principle III firewall). Cross-reference: `specs/008-m5-structural-passes/research.md` §20 documents the M5-side reasoning; `specs/008-m5-structural-passes/spec.md` Assumptions bullet for "M4-frozen op set" is unchanged at 79.

### Session 2026-05-02 (post-merge amendment #4)

- Q: M5 implementation of US2 (`NSLResolveParamsPass`, `NSLExpandGeneratePass`) and US3 (`NSLExplodeSubmodArrayPass`) surfaced a bundle of three structural gaps with the M4 dialect (post-amendment-#3 freeze surface 77): (a) NSL grammar (lang.ebnf §3.1) places `param_int N = 8;` / `param_str WIDTH = "8";` at compilation-unit top level (sibling of `module B { ... }` per S16), but the M4 dialect carried no top-level parameter ops, leaving `NSLResolveParamsPass` slot 1 with no IR target to consume (Principle VII coupling violation: 6 M5 contracts referenced these ops); (b) `nsl.structural_generate` carried no loop-variable-name attribute, so M5's `NSLExpandGeneratePass` had no target for `%IDENT%` macro substitution; (c) `nsl.submodule` had no array form, so M5's `NSLExplodeSubmodArrayPass` (slot 4 per design §9) had no IR shape to consume per FR-016. Choose between (A) emit CIRCT-side helpers directly from M5 (violates Principle III: M5 lowers to the `nsl` dialect, not skip ahead to CIRCT); (B) amend M4 to add the bundled three changes — two new ops (`nsl.param_int`, `nsl.param_str`) plus two field additions (`structural_generate.loop_var`, `submodule.array_size`); (C) defer the structural-expansion passes to M6 (semantically opaque IR; breaks the M5→M6 cut); (D) downgrade to stub-only ops (postpones the M5→M6 cut and breaks Principle VIII test sequencing). → A: **Option (B) — amend M4 with the bundled three changes.** Rationale: (A) violates Principle III's architectural seam — M5 lowers AST → `nsl` dialect; the dialect must contain everything M5 produces. (C) makes `-emit=mlir` semantically meaningless for the structural-pass interim shape. (D) breaks the FileCheck-on-real-IR test discipline. Cost: 2 new op records (TableGen + assemblyFormat ~30 LOC; both Symbol-bearing with `HasParent<"::mlir::ModuleOp">`) + 2 OptionalAttr additions on existing op records + 4 new round-trip fixtures (`test/Dialect/storage/{param_int,param_str,submodule_array}_roundtrip.mlir` + `test/Dialect/expansion-only/structural_generate_loopvar_roundtrip.mlir`); freeze surface grows 77 → 79 per `contracts/dialect-api.contract.md` §2 post-merge note #4; SC-012's "next op" baseline updates from "71st op" to "73rd op". The OptionalAttr additions on `nsl.structural_generate` and `nsl.submodule` are field-only (no new op classes — existing fixtures using the old shape continue to round-trip exactly because the optional default round-trips to the same printed form). CIRCT-side conversion code does NOT land at M4 — design §10 documents the M6 mapping per-op (Principle III firewall: M4 dialect is the seam, NOT CIRCT); the array-form `nsl.submodule` maps to per-element `hw.instance`, `nsl.param_int` maps to a Verilog/VHDL/SystemC parameter wire on the resulting `hw.module` instance, `structural_generate.loop_var` has no CIRCT counterpart since `nsl.structural_generate` is M5-eliminated. Cross-reference: `specs/008-m5-structural-passes/research.md` §18 documents the M5-side reasoning; `specs/008-m5-structural-passes/spec.md` Assumptions bullet for "M4-frozen op set" updates the count from 77 to 79.

### Session 2026-05-02 (post-merge amendment #3)

- Q: M5 lowering of `StructCastExpr` / `FieldAccessExpr` (T043 + T044) surfaced a structural gap with the M4 dialect: NSL grammar places `struct S { ... }` at compilation-unit top level (sibling of `module B { ... }` per `lang.ebnf §1`), but `nsl.struct` carries `HasParent<"ModuleOp">` — forcing the AST→nsl seam to invent a synthetic enclosing `nsl.module` for every top-level struct. Choose between (i) keep the strict parent + synthesise a wrapping `nsl.module` for every struct in the AST→nsl seam (clutters every printed IR with synthetic modules; violates the "structurally faithful" lowering principle established by Q1 Option A); (ii) relax the `nsl.struct` parent trait from `HasParent<"ModuleOp">` to `ParentOneOf<["::mlir::ModuleOp", "ModuleOp"]>` so top-level placement under the builtin `mlir::ModuleOp` is legal; (iii) move structs into a per-module child slot (contradicts NSL grammar; would force every cross-module struct reference to use a multi-segment SymbolRef). → A: **Option (ii) — relax the parent trait.** Change `nsl.struct`'s trait from `HasParent<"ModuleOp">` to `ParentOneOf<["::mlir::ModuleOp", "ModuleOp"]>`. Rationale: (i) clutters every printed IR with synthetic modules and forces M6 lowering to detect-and-discard them; (iii) re-encodes the struct hierarchy at IR-level in a way that contradicts NSL grammar's flat top-level layout. **Purely additive** — existing module-scoped struct fixtures (`test/Dialect/Types/struct_roundtrip.mlir`, `test/Dialect/module-level/struct_roundtrip.mlir`, both `struct_invalid_*` fixtures) continue to verify and round-trip; no new ops introduced, freeze surface unchanged at **77**. The per-op invariant row for `nsl.struct` updates from `parent = ModuleOp` to `parent ∈ {::mlir::ModuleOp, ModuleOp}` (data-model §2.1). `StructOp::verify()` is unchanged — its field-cycle check uses `mlir::SymbolTable::getNearestSymbolTable(*this)` which already resolves correctly when the struct is a sibling of `nsl.module` under the builtin `mlir::ModuleOp` (which itself implements `SymbolTable`). Sibling consumers (`StructCastOp::verify`, `FieldOp::verify`) likewise use `getNearestSymbolTable` and require no amendment. Round-trip fixture: `test/Dialect/Types/struct_toplevel_roundtrip.mlir`. SC-012's "next op" baseline is unchanged ("71st op"). Cross-reference: `specs/008-m5-structural-passes/research.md` §17 documents the M5-side reasoning.

### Session 2026-05-02 (post-merge amendment #2)

- Q: M5 expression-lowering work surfaced a second-order structural gap: spec FR-007 (M5) mandates that `BinaryExpr`, `UnaryExpr`, `ConditionalExpr`, `SliceExpr`, and `ConcatExpr` lower to `mlir::Value`, but the M4 dialect (post-2026-05-01 amendment, freeze surface 49) has no expression-position ops at all — only `nsl.constant`, the 41 structural ops, and the 4 marker / lowering-helper ops. Design §10 implies the per-op CIRCT mapping (`nsl.add → comb.add` etc.) lands at M6, but if those ops don't exist at M4, M5's `lowerExpr` has nothing to construct. Choose between (A) implement FR-007 by emitting CIRCT `comb.*` ops directly from M5 (skip the `nsl` IR layer for expressions); (B) amend M4 to add the full expression-op surface (28 ops covering arith / bitwise / shift / comparison / logical / unary / extend / mux / concat / extract / repeat); (C) defer FR-007 to M6 (cross-dialect placeholder ops at M5); (D) downgrade FR-007 to "stub-only — emit `nsl.unknown_expr` markers", deferring real expression-IR until a later milestone. → A: **Option (B) — amend M4.** Add the 28-op expression surface in 6 cluster-commits (cluster 1+3+4 = 8 trait-covered binary ops; cluster 2 = 8 cmp+logical ops; cluster 5 = 6 unary ops; cluster 6 = 2 extend ops; cluster 7a = mux + concat; cluster 7b = extract + repeat). Rationale: (A) violates Principle III's architectural seam — M5 lowers AST → `nsl` dialect; the dialect must contain everything M5 produces. (C) creates cross-dialect sleight-of-hand and clouds every printed IR. (D) postpones the M5→M6 cut and breaks Principle VIII test sequencing (FileCheck fixtures need the real IR shape, not stubs). Cost: 28 new op records (TableGen ~250 LOC + verifier ~150 LOC + 28 round-trip fixtures + 16 verifier-reject fixtures for the ops with non-trait-covered invariants); freeze surface grows 49 → 77 (per `contracts/dialect-api.contract.md` §2 post-merge note #2); SC-012's "next op" baseline updates from "43rd op" to "71st op". Per-cluster hand-verifier scope: clusters 1+3+4 are trait-covered (Pure + SameOperandsAndResultType + Commutative where applicable; no body); cluster 2 ops verify result is `!nsl.bits<1>` (logical ops additionally verify operand width 1); cluster 5 reductions + lnot verify result `!nsl.bits<1>`; cluster 6 verifies result-width ≥ operand-width; cluster 7a `nsl.mux` verifies cond-width-1 + then/else/result type-equal; cluster 7a `nsl.concat` verifies result-width = sum-of-operand-widths; cluster 7b `nsl.extract` verifies `lowBit + result_width ≤ operand_width`; cluster 7b `nsl.repeat` verifies `result_width = count × operand_width`. CIRCT-side conversion code does NOT land at M4 — design §10 documents the M6 mapping per-op (Principle III firewall: M4 dialect is the seam, NOT CIRCT). Cross-reference: `specs/008-m5-structural-passes/research.md` §16 documents the M5-side reasoning; `specs/008-m5-structural-passes/spec.md` Assumptions bullet for "M4-frozen op set" updates the count from 49 to 77.
