...
- [~] T086 [US1] **DEFERRED 2026-04-30 to user-side build verification** — the Phase 4 implementer could not invoke the dev container build inside the agent sandbox (`sg docker -c "docker run …"` requires elevated privilege bypass that the user did not authorize this turn). All TableGen records, dialect cpp wiring, fixture-coverage data, and lit.cfg fixes have landed; the implementation should compile clean inside `ghcr.io/koyamanx/nsl-nslc:dev`. **Recommended user action**: run `./scripts/ci.sh all` (or `cmake --build build-Release-noasan --target check-nslc`) to verify: 41 round-trip op fixtures + 3 type round-trip fixtures pass; `scripts/check_dialect_coverage.py` confirms paired round-trip fixtures; `dialect_register_test` gtest stays green; M0–M3 corpus stays green. Per FR-017, FR-018, FR-021, SC-001, SC-003, SC-004.
