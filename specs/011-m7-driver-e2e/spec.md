...
- **FR-017**: For each of the four CPU projects (`cpu16`, `mips32_single_cycle`, `rv32x_dev`, `turboV`), the golden VCD's provenance MUST be either (a) a manually-authored reference trace from a known-good instruction stream + cycle-accurate hand-traced expected register/memory updates, or (b) a formal-validation framework export (e.g. riscv-formal trace export — applicable to `rv32x_dev` and `turboV` once M8 lands; pre-M8, manually-authored is the only option for these).
