...
- [ ] T032 [US1] Run `cd grammars/treesitter && npx tree-sitter generate` to produce updated `parser.c`, `grammar.json`, `node-types.json`; commit all three; verify `parser.c` is byte-identical to a second regeneration run (regenerate-and-diff readiness). **DEFERRED IN-SESSION**: depends on T002 + T012 (`npm install` + initial `tree-sitter generate`); runs inside the dev container alongside T034
- [X] T033 [US1] Author `grammars/treesitter/queries/highlights.scm` with the FR-007 required-minimum 20-capture set per `contracts/highlights-coverage.contract.md` §1: keyword captures (#1–#5 sourced from `_keyword` token kinds), `@type.builtin` (#6) and `@type` (#7), `@function.call` (#8), `@constant.macro` (#9), `@number`/`@string`/`@comment` (#10–#12), and the eight Q3-locked sub-captures (#13–#20: `@variable.register`/`@variable.wire`/`@variable.memory`, `@function.proc`/`@function.func`, `@function.call.proc`/`@function.call.func`, `@label.state`); plus the FR-009 dedicated control-terminal capture; SPDX line-1 `; SPDX-…` header. **Outcome**: structured top-to-bottom GENERIC-first / SPECIFIC-last (tree-sitter's last-match-wins policy); decl-site captures via field bindings; operator-side LHS captures via `register_transfer` vs `wire_or_variable_transfer` rule discrimination; call-site captures via `control_call` (statement-position) vs `call_expression` (expression-position) discrimination — both precise by syntactic context. Reference-site resolution closed by `grammars/treesitter/queries/locals.scm` (added in the same series) per the tree-sitter-highlight `@local.scope`/`@local.definition.<NAME>`/`@local.reference` convention — both initially-deferred assertions (RHS `q` in `w = q;` and S27 `work` in `busy = work;`) are now covered. Focused SPDX check passes (6/6 — `highlights.scm` + `locals.scm` + four others)
- [X] T033a [US1] Author `grammars/treesitter/queries/locals.scm` — scope + definition + reference queries that close the two RHS-reference assertions in `reg_vs_wire.nsl` and `control_terminal_s27.nsl` without binding-tracking code outside tree-sitter. **Outcome**: 13 scope rules covering module_block / declare_block / proc_definition / state_definition / func_definition / seq_block / parallel_block / for_block / while_block / alt_block / any_block / init_block / structural_generate; 12 definition rules keyed by the highlight-name suffix (e.g. `@local.definition.variable.register`, `@local.definition.function.proc`, `@local.definition.variable.builtin.terminal`); single broad `(identifier) @local.reference` reference rule per the tree-sitter-rust / tree-sitter-go / tree-sitter-c convention. `contracts/highlights-coverage.contract.md` §1 amended with a "Reference-site resolution mechanism" paragraph documenting the locals.scm extension (Principle VII coupling)
- [ ] T034 [US1] Run `cd grammars/treesitter && npx tree-sitter test` and verify ALL six goldens from T016–T021 now PASS; if any assertion fails, iterate on `grammars/treesitter/queries/highlights.scm` (T033) / `locals.scm` (T033a) or grammar productions (T023–T030 → re-run T031–T032). **DEFERRED IN-SESSION**: depends on T002 + T012 + T032; runs inside the dev container. With `locals.scm` in place, every assertion in every fixture should resolve cleanly via tree-sitter-highlight's scope walker; if any of the four golden-fixture RHS-reference assertions still fail (the two flagged in T033a outcome + transitive cases), the dev-container iteration adjusts the scope list or moves a reference position into an explicit `highlights.scm` pattern
