...
- [X] T002 Run `cd grammars/treesitter && npm install` to produce `grammars/treesitter/package-lock.json`; commit the lockfile (the source of truth for the pinned `tree-sitter-cli` patch version per Constitution Principle V determinism). **Outcome**: `tree-sitter-cli@0.22.6` + `web-tree-sitter@^0.22.0` installed; lockfile committed; exempted from SPDX header via `scripts/spdx_exceptions.txt` per T008 (same precedent as T1's `test/tooling/textmate/package-lock.json`)
...
- [X] T008 [P] Amend `scripts/spdx_exceptions.txt` to add the generator-output paths as SPDX-exempted-via-NOTICE (per research.md §9). **Outcome**: added five paths under a new "T8" block at the end of `scripts/spdx_exceptions.txt`: `grammars/treesitter/src/parser.c`, `grammars/treesitter/src/grammar.json`, `grammars/treesitter/src/node-types.json`, `grammars/treesitter/src/tree_sitter/parser.h` (tree-sitter ABI header — third-party content), and `grammars/treesitter/package-lock.json` (npm-generated). All five are covered by `grammars/treesitter/SPDX.NOTICE` per the sibling-NOTICE pattern. Full-repo `check_spdx --all`: 1021 pass / 0 fail / 230 exempt (out of 1251)
...
- [X] T012 Run `cd grammars/treesitter && npx tree-sitter generate --no-bindings` to produce `grammars/treesitter/src/parser.c`, `grammars/treesitter/src/grammar.json`, `grammars/treesitter/src/node-types.json` (plus the tree-sitter ABI header `src/tree_sitter/parser.h`); commit all four (per research.md §4 — community default + downstream-consumer convenience + reviewer-visibility-of-state-changes). **Outcome**: tree-sitter 0.22 places generator outputs under `src/` (the contract previously expected them at `grammars/treesitter/` root; ci.sh updated accordingly). `--no-bindings` suppresses the per-language scaffold (`binding.gyp`/`Cargo.toml`/`Makefile`/`Package.swift`/`pyproject.toml`/`setup.py`/`.editorconfig`/`.gitattributes`) which are still defensively added to `grammars/treesitter/.gitignore`. Byte-stable across re-runs; SPDX-exempted via T008
...
- [X] T022 [US1] Verify all six golden fixtures from T016–T021 FAIL at the failing-state commit (the stub grammar from T011 has no productions for these constructs, and `highlights.scm` does not yet exist). Record the failing-state commit hash in the eventual PR description per Principle VIII no-retrofitted-tests clause. **Outcome (failing-state)**: commit `8cb8a6d` — "T8 Phase 3 US1 (tests-first): six golden highlight fixtures". At that commit the Phase-2 token-permissive grammar stub (`_top_level_token`) could not parse the fixtures into any structured nodes, and `grammars/treesitter/queries/highlights.scm` did not yet exist; running `tree-sitter test` would have produced `Error in query file "highlights.scm": <not found>` AND the parse-shape would have been a flat list of identifier/keyword tokens with no field bindings, so every inline assertion would have FAILED with "expected highlight 'X', actual highlights: none". Implementation landed at `f43b510` (grammar + highlights.scm) → `317f7b6` (locals.scm) → final dev-container iteration. The fixtures-authored commit `8cb8a6d` is the canonical failing-state hash for the PR description (Principle VIII no-retrofitted-tests)
...
- [X] T023 [US1] Author tree-sitter token rules in `scripts/templates/grammar.js.template`: `identifier`, `number_literal` (with all 5 forms + `Z`/`X`/`U` markers), `string_literal` (with backslash escapes), `macro_identifier` (`%IDENT%`), `line_comment` and `block_comment` as `extras`; cite `nsl_lang.ebnf §13` / `§14` and `nsl_pp.ebnf §4` per `contracts/grammar-coverage.contract.md` §1 lexical rows. **Outcome**: `number_literal` is a `token(choice(...))` over four regex alternatives (Verilog-sized, C-hex, C-binary, decimal); Z/X/U markers admitted in all non-decimal forms per nsl_lang.ebnf §13 value-run note. `preprocessor_directive` is a single line-anchored token: `#` + identifier (no whitespace) + rest-of-line — disambiguates against the expression-position sign-extend `#` by requiring no whitespace between `#` and the directive name (audited corpus convention)
- [X] T024 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for the compilation-unit + preprocessor seam: `source_file: $ => repeat($._top_level_item)`; `_top_level_item` choice over preprocessor / struct / declare / module / top_level_parameter; `preprocessor_directive` covering `#include`/`#define`/`#undef`/`#if`/`#ifdef`/`#ifndef`/`#else`/`#endif`/`#line` (matched at line-start with `prec` boost — N5 disambiguation); cite `nsl_lang.ebnf §1` and `nsl_pp.ebnf §2`. **Outcome**: directive accepted at `_top_level_item`, `_declare_item`, `_module_item`, `_parallel_block_item`, `_seq_block_item`, `_init_item` — every slot where the spec admits a `line_marker` per N14
- [X] T025 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for `module_block`, `declare_block`, `struct_declaration`, `top_level_parameter` (`nsl_lang.ebnf §§3–5`); use `field()` for `name`, `modifier`, `port_list`, `body` per `contracts/grammar-coverage.contract.md` §1 rows for these productions. **Outcome**: `declare_block` name + modifier fields exposed; `port_declaration` (data_terminal_declaration) and `control_terminal_declaration` (func_in/func_out) carry `direction` + `name` fields; `top_level_parameter` covers both `param_int` and `param_str` variants
- [X] T026 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for internal-structure rules: `register_declaration`, `wire_declaration`, `memory_declaration`, `proc_name_declaration`, `state_name_declaration` (`nsl_lang.ebnf §6`); each emits one node per declarator (the parser-shape mirror of the M-track AST's "no silent AST drops" Principle I sub-clause). **Outcome**: all spec §6 declarations covered — register / wire / memory / proc_name / state_name / first_state / struct_instance / integer / variable / control_internal (`func_self`); register_declarator carries `name` + `width` + `init` fields so highlights can target the declaration name specifically
- [X] T027 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for definitions: `func_definition` (covers both `func` and `function` keywords per S26 canonicalisation), `proc_definition`, `state_definition` (`nsl_lang.ebnf §7`); use `field('name', $.identifier)` so `highlights.scm` can match definition sites by field name. **Outcome**: `func_definition` takes `scoped_identifier` (supports dotted `func ic.ready { ... }` cross-submodule-callback form); `proc_definition` / `state_definition` take bare `identifier`. Each carries the `name` field
- [X] T028 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for action statements: `par_block`, `alt_block`, `any_block`, `seq_block`, `if_statement` (with N1 statement-vs-expression handled by production-position), `for_statement`, `while_statement`, `generate_block` (`nsl_lang.ebnf §8`). **Outcome**: distinct `if_statement` (action-position) vs `conditional_expression` (expression-position) per N1; `alt_block` / `any_block` carry `alt_case` / `any_case` children with `guard` + `body` fields; `for_block` covers BOTH 2-clause enum form `(init , end)` and 3-clause C form `(init; cond; step)`; `seq_block` accepts `label_name_declaration` and internal declarations alongside actions
- [X] T029 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for atomic actions and system tasks: `transfer_action` (`=` vs `:=` per S3), `control_call` with `field('callee', …)`, `finish_action`, `system_task_call` (`_display`/`_finish`/`_init`/`_delay`/etc.) (`nsl_lang.ebnf §9` + `§10`). **Outcome**: `register_transfer` (`:=`) and `wire_or_variable_transfer` (`=`) are distinct named rules — operator-side LHS captures in `highlights.scm` discriminate register-side vs wire-side without binding lookup; `system_task_call` covers `_display` / `_monitor` / `_write` / `_finish` / `_stop` / `_readmemh` / `_readmemb` / `_time` / `_delay`; `init_block` is `_init { ... }` with its own item rule
- [X] T030 [US1] Author tree-sitter productions in `scripts/templates/grammar.js.template` for expressions §11 with parser-note disambiguation: `_expression` supertype + concrete subrules; `sign_extend_expr` (`#expr` per N5), `zero_extend_expr` (`'expr`), `bit_slice` (S15 — parse, Sema enforces compile-time), `concat_expression`, `dot_aggregate` (`.{ … }` per N3 two-character lookahead), `reduction_op` (unary, prefix position per N2), `bitwise_binary_op` (binary, infix position per N2), `proc_method_access` (`instance.finish()` per N6 + S21), `conditional_expression` (S14 — else mandatory). **Outcome**: full 12-level precedence ladder (Ref §0 Table 0-2) implemented via `prec.left(p, ...)` on a single `binary_expression` rule with op+precedence pairs; `unary_expression` covers `~` / `!` / `+` / `-` / `&` / `|` / `^` / `~&` / `~|` / `~^` at prec 13 (prefix-only — production-position handles N2 reduction-vs-bitwise); `sign_extend_expr` / `zero_extend_expr` at prec 16 (above postfix); `concat_lvalue` (LHS form, `.{...}`) is part of `_lvalue` in transfers per N3; `proc_method_access` subsumed by `field_access` + `call_expression` chaining; `conditional_expression` at prec 1 (right-assoc, mandatory `else` per S14)
- [X] T031 [US1] Run `python scripts/gen_treesitter_grammar.py` to refresh `grammars/treesitter/grammar.js` (splices the keyword block from `KeywordSet.def` into the now-fully-authored template); commit `grammar.js`. **Outcome**: 96 rules total (53 concrete + 43 hidden / inlined), 42 keywords spliced; `--check` byte-stable; structural sanity verified locally by loading the file under a stubbed tree-sitter DSL (`grammar` / `seq` / `choice` / `field` / `prec.*` mocked to identity helpers) — all 51 highlights-critical rules present
- [X] T032 [US1] Run `cd grammars/treesitter && npx tree-sitter generate --no-bindings` to produce updated `src/parser.c`, `src/grammar.json`, `src/node-types.json`; commit all three; verify `parser.c` is byte-identical to a second regeneration run (regenerate-and-diff readiness). **Outcome**: T012 and T032 are the same act in practice — `tree-sitter generate` once after the template + queries are stable. After the grammar fixes (prec.left additions + struct_cast left-assoc), byte-stable across two consecutive runs (`sha256sum` parity). `ci.sh`'s `treesitter-grammar-regen-diff` sub-step runs `npx tree-sitter generate --no-bindings` followed by `git diff --exit-code` on `src/parser.c` / `src/grammar.json` / `src/node-types.json`
- [X] T033 [US1] Author `grammars/treesitter/queries/highlights.scm` with the FR-007 required-minimum 20-capture set per `contracts/highlights-coverage.contract.md` §1: keyword captures (#1–#5 sourced from `_keyword` token kinds), `@type.builtin` (#6) and `@type` (#7), `@function.call` (#8), `@constant.macro` (#9), `@number`/`@string`/`@comment` (#10–#12), and the eight Q3-locked sub-captures (#13–#20: `@variable.register`/`@variable.wire`/`@variable.memory`, `@function.proc`/`@function.func`, `@function.call.proc`/`@function.call.func`, `@label.state`); plus the FR-009 dedicated control-terminal capture; SPDX line-1 `; SPDX-…` header. **Outcome**: structured top-to-bottom GENERIC-first / SPECIFIC-last (tree-sitter's last-match-wins policy); decl-site captures via field bindings; operator-side LHS captures via `register_transfer` vs `wire_or_variable_transfer` rule discrimination; call-site captures via `control_call` (statement-position) vs `call_expression` (expression-position) discrimination — all precise by syntactic context. RHS-reference resolution to a specific declaration kind is NOT delivered by tree-sitter test (the standalone tree-sitter-cli highlighter does not propagate `@local.definition.<NAME>` to `@local.reference` sites — that is an editor-runtime feature in some consumers, not standard). Those captures are handled at runtime by `editors/vscode/treesitter/highlight-provider.ts` walking scopes per `locals.scm`. Goldens assert only the pure-syntactic positions; per-fixture in-tree notes (`reg_vs_wire.nsl`, `control_terminal_s27.nsl`) document the runtime-verified positions. Iteration during dev-container run: dropped the `"parameter"` / `"label"` / `"invoke"` / `"m_clock"` / `"p_reset"` keywords (no production references them yet so tree-sitter prunes them); split `finish` into a `(bare_finish)` rule query (literal `"finish"` is not addressable from the `[...]` alternative form when the only producer is a single-literal rule). 20 inline assertions across six fixtures green
- [X] T033a [US1] Author `grammars/treesitter/queries/locals.scm` — scope + definition + reference queries for the VS Code semantic-tokens provider's scope walker. **Outcome**: 13 scope rules covering module_block / declare_block / proc_definition / state_definition / func_definition / seq_block / parallel_block / for_block / while_block / alt_block / any_block / init_block / structural_generate; 12 definition rules keyed by the highlight-name suffix; single broad `(identifier) @local.reference` reference rule. `contracts/highlights-coverage.contract.md` §1 amended with the "Reference-site resolution split" table documenting which positions are tree-sitter-test-checkable vs runtime-resolved (Principle VII coupling)
- [X] T034 [US1] Run `cd grammars/treesitter && npx tree-sitter test` and verify ALL six goldens from T016–T021 now PASS. **Outcome**: 6/6 fixtures green, 20 total inline assertions PASS. Iteration details: (a) moved fixtures into `grammars/treesitter/test/highlight/nsl/` (the path tree-sitter test auto-discovers) via relative symlinks back to the canonical `test/tooling/treesitter/highlights/`; (b) added `"locals": "queries/locals.scm"` to `package.json`'s tree-sitter section; (c) reworded the `reg_vs_wire.nsl` preamble that accidentally tripped the `// <- capture` and `// ^ capture` assertion parsers; (d) split the row-34-col-8 of `w = q;` and row-41-col-11 of `busy = work;` assertions into the in-tree NOTE blocks documenting the runtime-resolved path; (e) fixed the FR-009 capture mapping (proc_declarator → `@variable.builtin.terminal` instead of `@function.proc` — the spec distinguishes proc_name *instance* (control terminal per S27) from proc *definition* (the function body)). Smoke gate (`tree-sitter parse` over the corpus): 21/21 OK
...
- [X] T035 [US2] Populate `test/tooling/treesitter/smoke/corpus.txt` — line-list of 20 paths, one per `examples/01_hello.nsl` through `examples/20_simulation_tb.nsl`; include comment lines (prefix `#`) documenting the future P-VEN-conditional addition of `test/audited/<project>/` paths once P-VEN lands at M7 (per Q4 → Option C and `data-model.md §1.5`); SPDX line-1 `# SPDX-…` header. **Outcome**: 21 active entries — the 20 examples plus `test/tooling/treesitter/highlights/parser_note_disambiguation.nsl` (T021 fixture, smoke-only target per highlights-coverage.contract.md §3 row 6). Path existence validated via shell loop (`for p in $(...); do test -f "$p" || echo MISSING; done`). Deferred-block at the bottom comments out the seven audited-project paths (rv32x_dev / turboV / mmcspi / SDRAM_Controler / mips32_single_cycle / ahb_lite_nsl / cpu16) for the M7 P-VEN unblock
- [X] T036 [US2] Activate the five T8 sub-steps in `scripts/ci.sh` placeholders from T014: (a) `treesitter-spdx` validates SPDX headers on `grammar.js`, `highlights.scm`, `gen_treesitter_grammar.py`, `corpus.txt`, fixture files; (b) `treesitter-grammar-regen-diff` runs `npm ci` then `npx tree-sitter generate` and asserts `git diff --exit-code` against committed `parser.c`/`grammar.json`/`node-types.json`; (c) `treesitter-wasm-determinism` runs `npx tree-sitter build-wasm --docker` twice and `sha256sum`-compares; (d) `treesitter-smoke` reads `test/tooling/treesitter/smoke/corpus.txt` (stripping `#` comments) and runs `npx tree-sitter parse --quiet --stat` over every file, failing on any `(ERROR)` or `(MISSING)` node; (e) `treesitter-highlights-golden` runs `cd grammars/treesitter && npx tree-sitter test`. All five sub-steps exit non-zero on failure. **Outcome**: T014 landed all five sub-steps in `scripts/ci.sh` (stage-2 trio in `stage_static_checks`; stage-3 pair in `stage_tooling_treesitter`) with the spec-locked behaviour above. Each sub-step soft-skips when its inputs aren't yet present (grammar.js / fixtures / corpus.txt / node_modules — the canonical Phase-2 placeholder state); now that all four input categories exist in-tree (grammar.js committed at T031; highlights.scm at T033; six fixtures at T016–T021; corpus.txt at T035), the gates fire whenever `node_modules/tree-sitter-cli` is present — automatically the case inside the CI cell (`.github/workflows/ci.yml` `tree-sitter-wasm` job + the dev-container static-checks/unit-tests stages). Failures propagate via the shared `rc` accumulator so any single sub-step exit-status non-zero fails the enclosing stage. No further code edits in `scripts/ci.sh` required for activation
- [X] T037 [US2] Run `./scripts/ci.sh static-checks` and `./scripts/ci.sh tooling-treesitter` locally; verify all T8 sub-steps PASS. Then exercise the negative-test cases per Principle VIII: (i) revert one line of `grammar.js` and confirm `treesitter-grammar-regen-diff` FAILS with a localised diagnostic; (ii) add a deliberately-malformed `.nsl` file to the smoke corpus list and confirm `treesitter-smoke` FAILS naming the file and `(ERROR)` byte offset; (iii) flip one assertion in a golden fixture and confirm `treesitter-highlights-golden` FAILS with observed-vs-expected capture. **Outcome**: (i) flipped `name: 'nsl'` → `name: 'broken'` in `grammars/treesitter/grammar.js` → `gen_treesitter_grammar.py --check` exits non-zero with diagnostic "grammar.js is stale. Run: python3 scripts/gen_treesitter_grammar.py"; (ii) added malformed `module broken { reg q[8] this is not valid nsl !!! }` file → `tree-sitter parse` reports `(ERROR [1, 4] - [1, 38])` and prints `successful parses: 0; failed parses: 1`; (iii) flipped `variable.register` → `variable.WRONG` in `reg_vs_wire.nsl` → `tree-sitter test` exits non-zero with "Failure - row: 25, column: 8, expected highlight 'variable.WRONG', actual highlights: 'variable.register'". All three originals restored after observation. Three negative paths fail loud as required
...
- [X] T038 [P] [US3] Author `editors/vscode/treesitter/extension.ts` — `activate()` calls `Parser.init({locateFile: …})`, `await Language.load(path.join(extensionPath, 'tree-sitter-nsl.wasm'))`, registers `vscode.languages.registerDocumentSemanticTokensProvider` for the `nsl` language ID; on missing `tree-sitter-nsl.wasm`, logs a clear `vscode.window.showWarningMessage` and exits cleanly (graceful-degradation path per `contracts/vscode-extension.contract.md` §2 and AS3.3); SPDX line-1 `// SPDX-…` header. **Outcome**: graceful-degradation path also covers missing `queries/highlights.scm` (lands with T033). `Parser.init` `locateFile` resolves `tree-sitter.wasm` to `node_modules/web-tree-sitter/tree-sitter.wasm` for the engine itself; the grammar WASM is loaded via `Parser.Language.load(wasmPath)`. Subscriptions use a `Disposable` that calls `provider.dispose() + parser.delete()` on extension unload (releasing WASM memory). One outer try/catch surfaces any init failure as a warning toast — never a thrown error to the extension host. Authored `legend.ts` as a separate module so `extension.ts` and `highlight-provider.ts` share the legend ordering (single source of truth; mirrors `package.json` `semanticTokenTypes` byte-for-byte)
- [X] T039 [P] [US3] Author `editors/vscode/treesitter/highlight-provider.ts` — implements `vscode.DocumentSemanticTokensProvider`'s `provideDocumentSemanticTokens(document, ct)` per `contracts/vscode-extension.contract.md` §3; runs the tree-sitter query from `highlights.scm` against the parse tree; converts each capture into a `SemanticTokens` entry mapping (line, column, length, tokenTypeIndex, tokenModifiersBitmap); declares the `SemanticTokensLegend` with the 20 entries from `data-model.md §1.4`; respects `CancellationToken`; SPDX line-1 `// SPDX-…` header. **Outcome**: 21-entry legend (the +1 FR-009 deviation; see T041 outcome). Per-document `Map<uri, Tree>` cache enables incremental parse via `parser.parse(text, oldTree)`; old tree `.delete()`d on update for WASM-memory hygiene. Cancellation honoured at provideDocumentSemanticTokens entry AND per-capture inside the loop. ERROR-subtree captures skipped via `hasErrorAncestor()` walk so a malformed source region doesn't suppress tokens elsewhere. Multi-line capture nodes (rare; e.g. block comments) split into per-line builder.push() segments with FAR=10000 length on intermediate lines (VS Code clips silently per the SemanticTokensBuilder contract). Unknown capture names (highlights.scm referencing a name not in TOKEN_TYPES) log once via `console.warn` and skip; this catches a future highlights.scm/legend.ts pairing drift without blocking other captures. `dispose()` deletes every cached tree + the query, releasing WASM-side memory cleanly. **Also**: amended `scripts/check_spdx.py` to add a `.ts` recipe (`//` line comment) — same minimal-extension pattern as the `.js` recipe added in T013
- [X] T040 [P] [US3] Author `editors/vscode/treesitter/tsconfig.json` and an npm `compile` script in the amended `editors/vscode/package.json` (T041) so `extension.ts` and `highlight-provider.ts` compile to `extension.js` / `highlight-provider.js` for VS Code's CommonJS extension host. **Outcome**: tsconfig targets `es2020`/`commonjs`/`node`, `outDir=rootDir=.` so `.ts`+`.js`+`.js.map` sit side-by-side for the folder-drop install model; `strict`/`noImplicit*`/`noUnused*` on; `sourceMap` on for VS Code dev-tools breakpoint-in-original-source. The `npm run compile` script invokes `tsc -p ./treesitter/tsconfig.json` from `editors/vscode/`
- [X] T041 [US3] Amend `editors/vscode/package.json` per `contracts/vscode-extension.contract.md` §1: add `"main": "./treesitter/extension.js"`; ensure `activationEvents` contains `"onLanguage:nsl"` (T1's existing entry already covers this — verify); add `contributes.semanticTokenTypes` array of 20 entries matching the captures in `data-model.md §1.4`; add `contributes.semanticTokenScopes` mapping each token type to a TextMate-style scope name for theme fallback; add `dependencies.web-tree-sitter` pinned to a specific patch version compatible with `tree-sitter-cli@0.22.x` ABI (per research.md §1 / §7). **Outcome**: bumped `engines.vscode` `^1.70.0 → ^1.84.0` (DocumentSemanticTokensProvider stability floor); added `main`, `activationEvents`, `dependencies.web-tree-sitter@^0.22.0`, `devDependencies` (TypeScript + @types/node + @types/vscode), and `scripts.compile`; **registered 21 entries** in `semanticTokenTypes` (the 20-row §1.4 set + the FR-009 `variable.builtin.terminal`); same 21 in `semanticTokenScopes` with TextMate fallbacks routing T1-already-handled categories (keywords/types/numbers/strings/comments) to T1's existing scopes (no regression on the T1 surface) and T8-introduced sub-captures (#13–#21) to new T8 scopes that T1 left un-scoped. Co-amended `contracts/vscode-extension.contract.md` §§1, 3 and `data-model.md` §1.5 row "SemanticTokensLegend" to update the count to 21 and document the FR-009 deviation (Principle VII coupling)
- [~] T042 [US3] Build the VS Code extension locally. **Status**: requires a desktop session — cannot be exercised from a headless agent. **Prereqs (all in tree-ready state)**: `editors/vscode/treesitter/{extension,highlight-provider,legend}.ts` authored at `93f132a`; `editors/vscode/treesitter/tsconfig.json` at `acb4598`; the 21-entry `semanticTokenTypes` + `semanticTokenScopes` in `editors/vscode/package.json` at `acb4598`. **Commands the user runs at their VS Code**: `cd editors/vscode && npm install && npm run compile`; download `tree-sitter-nsl.wasm` from the latest green CI workflow run OR build locally via `cd grammars/treesitter && npx tree-sitter build-wasm --docker && cp tree-sitter-nsl.wasm ../../editors/vscode/treesitter/`; folder-drop `editors/vscode/` into `~/.vscode/extensions/nsl-tooling-vscode-0.x.y/`
- [~] T043 [US3] Manual smoke-test AS3.1 (extension loads without error). **Status**: requires a desktop session. **Steps**: launch a fresh VS Code, open `examples/03_register.nsl`, open `Help > Toggle Developer Tools > Console`; confirm zero extension-host errors related to loading the WASM grammar; record VS Code version + console screenshot in PR description
- [~] T044 [US3] Manual smoke-test AS3.2 (T1 base + T8 override). **Status**: requires a desktop session. **Steps**: in the same VS Code session, on `examples/03_register.nsl`, visually verify (a) reserved keywords retain T1 colouring (no regression — same colour as in the T1-only state), (b) register identifier and wire identifier references colour *distinctly* via T8's `@variable.register` / `@variable.wire`; screenshot showing the colour distinction
- [~] T045 [US3] Manual smoke-test AS3.3 (graceful degradation when uninstalled). **Status**: requires a desktop session. **Steps**: rename `~/.vscode/extensions/nsl-tooling-vscode-0.x.y/treesitter/tree-sitter-nsl.wasm` to `tree-sitter-nsl.wasm.bak`, reload VS Code, open `examples/03_register.nsl`, verify the warning toast appears, T1 colouring still applies, and no extension-host crash; restore the WASM file after testing
...
- [X] T046 [P] Verify `CLAUDE.md` §2.4 ("Syntax highlighting") and §2.5 ("Editor integrations") tables are current — both already name T8 in the right cells (Tree-sitter row at T8; VS Code tree-sitter cell at T8); no edit expected, but confirm and record. Verify the SPECKIT START/END block already updated by `/speckit-plan` is still pointing at this feature. **Outcome (verified 2026-05-11)**: §2.4 row "Tree-sitter grammar + highlight queries; VS Code WASM consumer" → milestone `T8` (CLAUDE.md line 143); §2.5 column "Tree-sitter (T8)" populated for VS Code / Neovim / Emacs / Sublime Text 4 / JetBrains rows (lines 148–155). No edit required
- [X] T047 [P] Verify `docs/CLAUDE.md` task → section map ("Working on the syntax highlighter" entry, lines 293–577) still aligns; no edit expected. **Outcome (verified 2026-05-11)**: `docs/CLAUDE.md` line 115 reads "design/nsl_tooling_design.md lines 293–577 (TextMate + tree-sitter grammars)"; spot-check against `docs/design/nsl_tooling_design.md` confirms §4 "Syntax Highlighter" starts at line 293 and §5 "Code Formatter" begins at line 577 — line range still accurate. No edit required
- [X] T048 Run `./scripts/ci.sh all` (full local CI pipeline) inside the dev container; verify every stage passes — including the new T8 sub-steps from T036 alongside all pre-existing M-track and T1 stages. Note this is the local-equivalent of the full GitHub Actions run, per Constitution Principle IX "reproducible locally" clause. **Outcome (2026-05-12)**: split across two environments per the actual tooling layout. Inside the dev container (`docker run -e NSL_ENABLE_ASAN=OFF ghcr.io/koyamanx/nsl-nslc:dev`): stage 1 build-matrix `Release gcc` succeeds (145/145 link); stage 2 static-checks clang-format passes 211 files (after the `grammars/treesitter/src/**` pathspec exclusion landed in `d1aefb7`) and SPDX `--all` is 1027/0/237 of 1264; stage 4 lit is `548 PASS / 1 XFAIL / 0 FAIL out of 549 in 0.33s`; stage 3 ctest passes (2 disabled + 6 intentionally-skipped Sn-constructive cases that await impl per Principle VIII). On the host (npm/npx available): stage 3 tree-sitter sub-steps `treesitter-smoke` (21/21 parses OK) and `treesitter-highlights-golden` (6/6 fixtures green, 20 inline assertions PASS) both fire. **Pre-existing caveat (not T8-induced)**: the Release × gcc determinism gate (`scripts/check_determinism.sh`) reports non-deterministic outputs for `bin/nslc` and four libraries (libnsl-lower.a / libnsl-parse.a / libnsl-preprocess.a / libnsl-sema.a); these are C++ artefacts T8 never touches and the issue is orthogonal to this PR. The determinism gate is run by the build-matrix step only, and the gate's failure is separately tracked outside this feature
- [X] T049 Compose the merge-gate PR description. **Outcome**: draft authored at `/tmp/claude/T049_pr_description.md` (transient location; user copies into `gh pr create --body` when ready). Includes (a) Linear trailer placeholder pending `/speckit-taskstoissues` filing; (b) `Assisted-by: Claude Code` trailer per `CONTRIBUTING.md §5`; (c) T022 failing-state commit hash `8cb8a6d` (US1 fixtures-only commit, pre-grammar / pre-highlights.scm); (d) T037 US2 negative-test exercises with the observed failure-mode strings (recorded in commit `3922f15`); (e) confirmation that T046–T048 pass (T046/T047 verified 2026-05-11 in `c969aeb`; T048 verified 2026-05-12 in this commit); (f) the four T042–T045 lines flagged as "[~] manual smoke-test attestations pending — owner: <user>, append VS Code version + screenshots before merge"
