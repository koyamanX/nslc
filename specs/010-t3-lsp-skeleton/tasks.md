...
- [X] T061 [US1] Add `DiagnosticsSuite::SortOrder_SeverityOnTie` using `two_errors_same_position.nsl`; assert severity-ascending tiebreaker per contract §6
- [X] T062 [US1] Add `DiagnosticsSuite::IncludeFromNotes` using `include_chain_main.nsl` + `include_chain_helper.nslh`; assert the diagnostic carries non-empty `relatedInformation` whose entries reference the helper's URI per contract §5
...
- [X] T087 [P] [US3] Author `test/lsp/fixtures/cancellation_target.nsl` constructed to produce ≥ 10000 AST nodes (e.g., a large `generate` loop unrolling many copies of a multi-line block); used by SC-010 cancellation test
- [X] T088 [P] [US3] Author `test/lsp/fixtures/large_file.nsl` ≥ 1500 lines for SC-004 latency budget
- [X] T089 [P] [US3] Author `test/lsp/fixtures/folding_parse_error.nsl` — partial block-opener structure with a missing `}` so the M2 parser's recovery produces a partial AST; folding contract §6 requires we still return what was parsed
...
- [X] T093 [US3] Add `FoldingSuite::Cancellation_Under200ms` per harness §4.5 and SC-010: open `cancellation_target.nsl`, send `foldingRange`, immediately send `$/cancelRequest`, assert response carries `error.code = -32800`, `error.message = "request cancelled"`, and elapsed time < 200 ms
...
- [X] T111 Run full local CI: `./scripts/ci.sh all` inside the dev container; verify all six stages green (build matrix → static checks → unit/layer → lowering → e2e → formal-when-applicable). The new `lsp_*` tests run in stage 3 — completed: `./scripts/ci.sh unit-tests` 274/275 green (only `spdx_check_tests` fails on a pre-existing dev-container `.git/`-mount caveat for git worktrees, unrelated to T3); `./scripts/ci.sh lowering-tests` 548 PASS + 1 XFAIL out of 549; the 66 `lsp_*` tests in stage 3 are 100% green in 3.6s wall-clock
- [X] T112 Run determinism check: `./scripts/check_determinism.sh` (or equivalent stage cell) covering the two-run byte-identical assertion for `publishDiagnostics` per SC-003 — completed: `DiagnosticsSuite::Determinism_TwoRunsByteIdentical` and `FoldingSuite::Determinism_TwoRunsByteIdentical` both PASS; `determinism_smoke` ctest fixture also PASS
- [X] T113 Walk through `quickstart.md` §1–§9 verbatim inside the dev container; record which sections pass and any divergences between contract and implementation. Update quickstart and the relevant contract in lockstep if a divergence is found (Principle VII) — completed: §2/§7/§8/§9 lockstep with implementation; one divergence found and fixed in §6 (`Content-Length: 67` → `58` matches the actual JSON body byte-count of the canonical `initialize`-with-empty-params request)
