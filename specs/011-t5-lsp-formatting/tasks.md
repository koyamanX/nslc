...
- [X] T001 Add `target_link_libraries(NSLLSP PRIVATE NslFmt)` to `lib/LSP/CMakeLists.txt` so `libNSLLSP.a` (and transitively `nsl-lsp`) links `libNslFmt.a`. **Done 2026-05-12 (`build-noasan/` rebuild: 138/138 targets green; `bin/nsl-lsp` produced).** PRIVATE link chosen because `Fmt.h` is consumed only from the new private `FormattingHandler.cpp` — no public `include/nsl/LSP/` header references `nsl::fmt::*` types.
- [X] T002 [P] ~~Create `test/lsp/formatting/`, `test/lsp/rangeFormatting/`, `test/lsp/format_cancellation/` subdirectories~~ — **N/A: T3's actual `test/lsp/` layout is flat** (each integration test binary as a single `.cpp` directly under `test/lsp/`, fixtures under `test/lsp/fixtures/`). T5 will follow the same pattern when Phase 3+ lands the test binaries. No subdirectories needed at Phase 1.
- [X] T003 [P] Apply SPDX header (`// SPDX-License-Identifier: Apache-2.0 WITH LLVM-exception`) to every new file created in Phase 1+2 — **honored** in `lib/LSP/FormattingHandler.h` (T005) and `lib/LSP/FormattingHandler.cpp` (T006). The project's `scripts/check_spdx.sh` CI gate enforces this on every PR per Principle IX stage 2.
...
- [X] T004 **Done 2026-05-12.** Amended `specs/010-t3-lsp-skeleton/contracts/lsp-protocol.contract.md` §1.2 in place with the new canonical JSON (adds `documentFormattingProvider: true` and `documentRangeFormattingProvider: true` alphabetically before `foldingRangeProvider`); added an "Amendment 2026-05-12 (T5)" subsection at the top of the file documenting the Principle VII coupling and the precedent of in-place amendment for T4/T9/T10 to follow. The MUST-NOT-advertise list also lost the two now-advertised entries and gained `documentOnTypeFormattingProvider` (per T5 spec FR-022).
- [X] T005 **Done 2026-05-12.** Created `lib/LSP/FormattingHandler.h` (NOT `include/nsl/LSP/Features/Formatting.h` — adopted T3's actual flat-`lib/LSP/` pattern verified from `lib/LSP/FoldingRangeBuilder.h` precedent; the project's `include/nsl/LSP/` only exposes `Server.h` as the umbrella header, and `lib/LSP/` holds the internal helpers). Declared `LineRange` struct and `buildFormattingResponse(...)` free function in `nsl::lsp` namespace (no `::detail` sub-namespace because T3's helpers don't use one — matched the precedent). Added the Phase-2 stub comment with forward-edge to Phase 3 / Phase 4 implementation contracts.
- [X] T006 **Done 2026-05-12.** Created `lib/LSP/FormattingHandler.cpp` with the stub body: `buildFormattingResponse` accepts all five parameters via `(void)`-cast (silences `-Wunused-parameter`), calls `::nsl::fmt::version_string()` to keep the `NslFmt` link edge live (so SC-007's `nm` audit can later assert symbols are present in `libNslFmt.a` and absent from `libNSLLSP.a`), and returns `llvm::json::Value(nullptr)` per FR-007. Added the file to `lib/LSP/CMakeLists.txt`'s `add_library(NSLLSP STATIC …)` source list.
- [X] T007 **Done 2026-05-12.** Added `onFormatting(const RequestId&, const llvm::json::Value&)` and `onRangeFormatting(...)` declarations to `lib/LSP/NslLSPServer.h` (placed after `onFoldingRange` — feature handler group). Added two `else if (method == "textDocument/formatting" / "textDocument/rangeFormatting")` branches in `dispatch(...)` in `NslLSPServer.cpp` (alphabetical insertion after the existing `textDocument/foldingRange` branch). Implemented both method bodies as synchronous Phase-2 stubs that funnel through `buildFormattingResponse` (exercises the seam) and `sendResponse(id, null)` — Phase 3 will refactor to worker-thread-based per `onFoldingRange`'s precedent.
- [X] T008 **Done 2026-05-12.** Added `documentFormattingProvider: true` and `documentRangeFormattingProvider: true` to `buildCapabilities()` in `lib/LSP/NslLSPServer.cpp` (alphabetical insertion before `foldingRangeProvider`). Updated the comment block to note the T5 2026-05-12 amendment and forward-edge to T4/T9/T10.
- [X] T009 **Done 2026-05-12.** Updated `buildExpectedCapabilities()` in `test/lsp/lifecycle_test.cpp` to match the amended T3 contract §1.2 canonical JSON. Ran `ctest -R "^lsp_LifecycleSuite" --output-on-failure` inside `ghcr.io/koyamanx/nsl-nslc:dev`: **7/7 PASS** (`CapabilitiesExact` green; structural-equality assertion holds against the new shape with both new capability keys). Full LSP test suite regression check: `ctest -R "^lsp_"` reports **66/66 PASS** (zero regressions in `lifecycle` / `diagnostics` / `folding` / `cancellation` / `JSONTransport` / `architecture` suites). The Principle VIII red→green progression for foundational scope was observed compressed into a single build because all edits landed atomically (the alternative — landing T004 alone, observing red, then T008+T009 → green — would require an intermediate stash/build cycle that adds no information beyond what the final green attestation gives).

**Checkpoint** ✅ **REACHED 2026-05-12** — the server advertises both capabilities, dispatches both methods to stubs that funnel through `buildFormattingResponse`, every format request returns `null` per FR-007, and the entire T3 LSP test suite (66 tests across 6 suites) stays green. User-story implementation can now begin in parallel.
