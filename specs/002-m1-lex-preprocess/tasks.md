...
- [X] T062 [US2] Build inside `ghcr.io/koyamanx/nsl-nslc:dev`: `cmake --build build-Release-gcc` + `ctest` + `lit build-Release-gcc/test`. **Result**: 90/90 lit tests + 65/66 ctest passing on `Release × gcc` (Phase-3 baseline). The single ctest failure (`spdx_check_tests`) is a pre-existing environmental issue — `git ls-files` fails inside the container because the worktree's `.git` lives outside the mounted volume; running `python3 scripts/check_spdx.py --all` on the host shows `237 passed, 0 failed`. Hand-verified: `#define`+`%IDENT%` splice (US2 scenario 5), helper evaluation `_int(_pow(2.0,8.0))` → `256` (scenario 3), `#include` with `-I` (scenario 1), `#if X > 3` branch selection (scenario 4), all four FR-037 locked diagnostics (P3, P6, P7, P9), `#line` round-trip with virtual-coordinate adjustment (scenario 8). Macro-table unit suite (T051) and helper-evaluator unit suite (T052) will turn green when the parallel agent's fixture corpus is merged in.
